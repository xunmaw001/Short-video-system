...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshipinxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshipinxinxiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshipinxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussshipinxinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshipinxinxi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshipinxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussshipinxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshipinxinxi', this.detail);
				this.$utils.msg('审核成功');
