<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.fabuzhanghao" type="text" placeholder="发布账号" ></input>
			</view>
                              			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.fabuxingming" type="text" placeholder="发布姓名" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
            ...
            <text v-if="userid && isAuth('fabuzhe','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('fabuzhe','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('fabuzhe','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('fabuzhe','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.fabuzhanghao="";
				this.searchForm.fabuxingming="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.fabuzhanghao){
					params['fabuzhanghao'] = '%' + this.searchForm.fabuzhanghao + '%'
				}
				if(this.searchForm.fabuxingming){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`fabuzhe`, params);
                } else {
                    res = await this.$api.list(`fabuzhe`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fabuzhe', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.fabuzhanghao){
					searchForm['fabuzhanghao'] = '%' + this.searchForm.fabuzhanghao + '%'
				}
				if(this.searchForm.fabuxingming){
					searchForm['fabuxingming'] = '%' + this.searchForm.fabuxingming + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`fabuzhe`, searchForm);
                } else {
                    res = await this.$api.list(`fabuzhe`, searchForm);
                }
				// 如果是第一页数据置空
