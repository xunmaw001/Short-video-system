...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','pindaofenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','pindaofenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('pindaofenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusspindaofenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('pindaofenlei', this.detail);
				this.$utils.msg('审核成功');
