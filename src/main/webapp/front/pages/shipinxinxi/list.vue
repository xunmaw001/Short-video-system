<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.pindaoleixing" type="text" placeholder="频道类型" ></input>
			</view>
                                                                                                                                    
			<view class="action">
				<button @tap="search" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'24rpx',color:'#fff',backgroundColor:btnColor[0],borderColor:btnColor[0]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
		</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                                                                  <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.shipintupian?baseUrl+product.shipintupian.split(',')[0]:''"></image>
                                                                                                                                                                          <view class="list-item-body" style="flex: 1;">
            <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.shipinmingcheng}}</view>
                                                                                                                                                                                                                                                                                                                      </view>
                                                                                                                                                                                                                                                          <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('shipinxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('shipinxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('shipinxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('shipinxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('pindaofenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('pindaofenlei', {page:1,limit:100});
            }
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.pindaoleixing = '%' + this.categoryName + '%'
				}

				if(this.searchForm.shipinmingcheng){
					params['shipinmingcheng'] = '%' + this.searchForm.shipinmingcheng + '%'
				}
				if(this.searchForm.pindaoleixing){
					params['pindaoleixing'] = '%' + this.searchForm.pindaoleixing + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`shipinxinxi`, params);
                } else {
                    params['sfsh'] = '是';
                    res = await this.$api.list(`shipinxinxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shipinxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.pindaoleixing){
					searchForm['pindaoleixing'] = '%' + this.searchForm.pindaoleixing + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`shipinxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`shipinxinxi`, searchForm);
                }
				// 如果是第一页数据置空
